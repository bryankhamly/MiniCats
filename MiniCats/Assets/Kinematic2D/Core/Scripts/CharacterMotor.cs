...
public class CharacterMotor : MotionCustomBehaviour
{
	...
	{	
		...
		{									
			...
			if( deltaPosition.y < 0)
			{
				...
				float verticalSlopeSignedAngle = Utilities.SignedAngle( currentVerticalDirection , collisionHitInfo.normal , characterBody.bodyTransform.Forward );
				...
				if( !groundSettings.alwaysNotGrounded )
				{
					...
					bool grounded = Utilities.BelongsToLayerMask( collisionHitInfo.gameObject.layer , verticalMask );
	...
	void FillGroundData( bool grounded , bool stable )
	{
		...
		collisionInfo.verticalSlopeSignedAngle = Utilities.SignedAngle( currentVerticalDirection , collisionHitInfo.normal , characterBody.bodyTransform.Forward );
	...
	void SetGroundDataSlopeAlignment( bool grounded , bool stable , GroundDetectionRay ray )
	{
		...
		collisionInfo.verticalSlopeSignedAngle = Utilities.SignedAngle( currentVerticalDirection , ray.normal , characterBody.bodyTransform.Forward );
		collisionInfo.verticalSlopeAngle = Mathf.Abs( collisionInfo.verticalSlopeSignedAngle );
		collisionInfo.verticalSlopeAngleSign = Mathf.Sign( collisionInfo.verticalSlopeSignedAngle );
	...
	{		
		...
		collisionInfo.wallSignedAngle = Utilities.SignedAngle( characterBody.bodyTransform.Up , collisionHitInfo.normal , characterBody.bodyTransform.Forward );
	...
	{	
		...
		float newSlopeSignedAngle = Utilities.SignedAngle( 
	...
	{				
		...
		if( groundAlignInfo.leftRay.collision && groundAlignInfo.rightRay.collision )
		{
			...
			rotationSignedAngle = Utilities.SignedAngle( characterBody.bodyTransform.Right , leftToRight , characterBody.bodyTransform.Forward );
		...
		if( !groundAlignInfo.leftRay.collision && groundAlignInfo.rightRay.collision )
		{
			rotationSignedAngle = Utilities.SignedAngle( characterBody.bodyTransform.Up , groundAlignInfo.rightRay.normal , characterBody.bodyTransform.Forward );
		...
		if ( groundAlignInfo.leftRay.collision && !groundAlignInfo.rightRay.collision )
		{						
			rotationSignedAngle = Utilities.SignedAngle( characterBody.bodyTransform.Up , groundAlignInfo.leftRay.normal , characterBody.bodyTransform.Forward );
	...
	void ProbeGroundRays( Vector3 deltaPosition )
	{
		...
		float verticalSlopeSignedAngle = Utilities.SignedAngle( currentVerticalDirection , testingRay.normal , characterBody.bodyTransform.Forward );
		float verticalSlopeAngle = Mathf.Abs( verticalSlopeSignedAngle );
	...
	{		
		...
		float signedAngle = Utilities.SignedAngle( characterBody.bodyTransform.Up , currentVerticalDirection , Vector3.forward);
	...
	void CheckDynamicGround()
	{
		if( dynamicGroundSettings.isEnabled )
		{
			if( IsGrounded && Utilities.BelongsToLayerMask( collisionInfo.groundLayer , layerMaskSettings.profile.dynamicGround ) )
			{	
	...
	void OnDrawGizmos()
	{
		...
		{
			Utilities.DrawArrowGizmo( characterBody.bodyTransform.Position , characterBody.bodyTransform.Position + 1.5f * GroundMovementDirection , Color.magenta );
		}
		...
		if( debugSettings.drawVerticalAlignmentGizmos )
		{
			...
			if( verticalAlignmentSettings.mode == VerticalAlignmentMode.Object )
			{
				if( verticalAlignmentSettings.verticalReferenceObject != null )
				{
					Utilities.DrawCross( verticalAlignmentSettings.verticalReferenceObject.position , 0.5f , Color.red );
					Utilities.DrawArrowGizmo( 
